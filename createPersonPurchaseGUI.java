...
public class createPersonPurchaseGUI extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent event) {
		if (event.getSource() == cancel)
			...
			if(progs.get(chosen) instanceof FlierProgram){
				createPurchaseFlyGUI window = new createPurchaseFlyGUI(traveler, progs.get(chosen));
				...
				createPurchaseGuestGUI window = new createPurchaseGuestGUI(traveler, progs.get(chosen));
				window.setLocation(this.getLocation());
				this.setVisible(false);
				window.showDialog(null, "Purchase Travel");
				this.setVisible(true);
	...
	public createPersonPurchaseGUI(Traveler trav){
		super();
		traveler = trav;
		...
		for(int i = 0; i<trav.size(); i++){
			progs.add(trav.getTravelProgram(i));
			progNames.add(trav.getTravelProgram(i).getProgramName());
		}
		centerPanel.add(new JLabel("Program name:"));
		programBox = new JComboBox(progNames.toArray());
		centerPanel.add(programBox);
		
		this.add(centerPanel,BorderLayout.CENTER);

		northText = new JLabel("Select Travel Program:");
